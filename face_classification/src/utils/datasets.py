...
class DataManager(object):
    ...
    def __init__(self, dataset_name='imdb', dataset_path=None, image_size=(48, 48)):
        ...
        elif self.dataset_name == 'CAS-PEAL-R1':
            self.dataset_path = '../datasets/POSE/'
        else:
    ...
    def get_data(self):
        if self.dataset_name == 'imdb':
            ...
            ground_truth_data = self._load_CAS_PEAL()
    ...
    def _load_CAS_PEAL(self):
        image_names = []
        gender_classes = []
        index_file_name = "FaceFP_2.txt"
        folder_names = self.make_folder_names()


        for names in folder_names :
            try :
                lines = open(self.dataset_path+names+"/"+index_file_name, "r").readlines()
            except FileNotFoundError :
                continue

            for image_name in lines :
                tok = image_name.split()
                image_names.append(names+"/"+tok[0]+".tif")
...
def split_CAS_PEAL_data(ground_truth_data, validation_split=.2, do_shuffle=False) :
